...
model_path = "D:/COCO/detection/out/fasterrcnn_resnet50_fpn_coco-258fb6c6.pth"
...
videopath = 'D:/COCO/NewYork_street.mp4'
...
outvideo = cv2.VideoWriter(videopath.replace(".mp4", "-Resnet.mp4"),fourcc,25.0,(416,416))
...
while(True):
    ...
    endtime = time.time()
    Fps = format(1/(endtime-starttime) , '.2f')
        ...
        for idx in range(boxes.shape[0]):
            ...
            if scores[idx] >= 0.8:
                ...
                cv2.putText(frame, text=text, org=(x1, y1-10), fontFace=cv2.FONT_HERSHEY_SIMPLEX, 
                    fontScale=0.3, thickness=1, lineType=cv2.LINE_AA, color=(0, 0, 255))
    cv2.putText(frame, text=Fps, org=(10,20), fontFace=cv2.FONT_HERSHEY_SIMPLEX, 
                    fontScale=0.8, thickness=1, lineType=cv2.LINE_AA, color=(0, 255, 0))
